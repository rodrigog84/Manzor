    ...
    public function reporte_detalle_productos_stock($idproducto,$mes,$anno)
      ...
      public function exportarExcelInventario()
       ...
       public function exportarExcelrecaudacion(){
        ...
        public function exportarExcelExistenciaselector()
         {
            header("Content-type: application/vnd.ms-excel"); 
            header("Content-disposition: attachment; filename=existencia.xls"); 
            
            $columnas = json_decode($this->input->get('cols'));
            $bodegas = json_decode($this->input->get('idbodega'));
            ...
            WHERE acc.id_bodega = "'.$bodegas.'"');

            $users = $query->result_array();

            foreach($users as $v){
                $nonbodega = $v['nom_bodega'];
            };
            
            echo '<table>';
            echo "<td></td>";
            echo "<td>LISTADO DE EXSTENCIA PRODUCTOS POR BODEGA</td>";
            echo "<tr>";
            echo "<td>BODEGA</td>";
            echo "<td>".$nonbodega."</td>";
            echo "<tr>";
                if (in_array("id", $columnas)):
                     echo "<td>ID</td>";
                endif;
